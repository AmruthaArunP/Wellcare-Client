...
function NavTopBar({ value }) {
  ...
  const handleLogout = () => {
    if (value === "doctor") {
      ...
      console.log("Before setUser(false):", user); // Add console.log statement
      setUser(false);
      console.log("After setUser(false):", user);
      navigate("/login");
    }
  };
    ...
    <>
      <nav className="bg-teal-500 max-w-screen mx-auto flex flex-wrap flex-col md:flex-row md:justify-between md:items-center mt-0">
        ...
        <div className="flex items-center mx-4">
          {(user && !value) || (doctor && value === "doctor") ? (
            <>
              <Link
                to={value === "doctor" ? "/doctor-profile" : "/user-profile"}
