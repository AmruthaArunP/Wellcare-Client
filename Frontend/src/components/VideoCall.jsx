...
function VideoCall({ value }) {
  ...
  const handleCallUser = useCallback(async () => {
    ...
    if (callActive) {
      ...
      setRemoteSocketId(null); //arun added
      ...
      socket.emit("user:call", { to: remoteSocketId, offer }); //arun changed??: CAN WE USE room ID instead of remoteSocketId
      setMyStream(stream);
      setCallActive(true);
    }
  ...
  const handleIncomingCall = useCallback(
    async ({ from, offer }) => {
      console.log("handleIncomingCall: setting remote socket ID: ", from);
  ...
  const handleNegoNeeded = useCallback(async () => {
    const offer = await peer.getOffer();
    console.log("handleNegoNeeded=> before calling socket.emit");
  ...
  const handleNegoIncoming = useCallback(
    async ({ from, offer }) => {
      const ans = await peer.getAnswer(offer);
      console.log("handleNegoIncoming=> before calling socket.emit peer:nego:done");
  ...
  return (
    <>
      <div className="bg-teal-200  text-center py-3 px-8 m-8  border rounded-lg shadow-lg">
        <h3 className="text-center text-4xl text-teal-600 font-bold mb-4 ">
          CONSULTING STATION
        </h3>
        {value === "user"
          ? !remoteSocketId && (
            <p className="mt-4">Please wait until the call arrives</p>
          )
          : !callActive && (
            <h5 className="mt-4">
              {remoteSocketId ? "Patient online" : "No one in the room"}
            </h5>
          )}
        <div className="container mx-auto mt-8">
          ...
          {callActive && (
            <button
              className="btn bg-red-500 text-white mt-4 border-2 px-4 py-2 rounded "
          ...
          {myStream && (
            <button
              className={`${!muted
                ? "bg-green-600 mt-4 border-2 px-2 py-2 rounded"
                : "btn-dark border-2 px-2 py-2 rounded"
                } mt-4 ms-3`}
              onClick={handleMute}
            >
          ...
          {value === "user" && myStream && (
            <button
              className={
                accepted
                  ? "hidden"
                  : "btn bg-green-600 text-white mt-4 border-2 px-8 py-2 rounded"
              }
              onClick={sendStreams}
            >
              Call
          ...
          {!callActive && value === "doctor" && remoteSocketId && (
            <button
              className=" bg-green-600 mt-4 border-2 px-2 py-2 rounded"
              onClick={handleCallUser}
            >
              Call
            </button>
